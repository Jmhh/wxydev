...
└── src
	...
	├── pages                                   #页面
	│   └── user                                  #用户页面
	│		├── rule-index.vue 					  #规则首页（当存在多个模块，已模块开头加功能描述）
	│   	├── detail.vue                        #用户详情页
	│       └── index.vue                         #用户主页
...
请求方法命名 以get获取、add添加、update更新、delete删除、
...
<<<<<<< HEAD
Vue文件 user-test.vue
css命名：外层容器 文件名+container 比如文件名是 login  class=“login-container”
=======
Vue文件、文件夹、css命名，尽量不要加复数s
例如 user-test.vue
>>>>>>> adbd20ae86e3e7a9851f8dcc163ae647e75fea72
